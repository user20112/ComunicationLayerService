    ...
    internal class ControlPackets
    {
        ...
        public void LoggingLevel(string message)
        {
            try
            {
                ...
                SNPService.DiagnosticOut("Logging Level Has been set to" + receivedPacket["LoggingLevel"].ToString(), 1);//Log Log Log everyone wants a log
        ...
        public void Silence(string message)
        {
            try
            {
                ...
                SNPService.DiagnosticOut("Sending Has been set to" + receivedPacket["Sendbool"].ToString(), 21);         //log it
                if (Convert.ToInt32(receivedPacket["IntTimeInSeconds"] ?? 0) != 0)                                      // if int time in seconds is set and not 0
                {
                    ...
                    if (OldSetting)
                        SNPService.DiagnosticOut("Sending Has been set to 1", 1);
                    else
                        SNPService.DiagnosticOut("Sending Has been set to 0", 1);
                }
            }
            catch (Exception ex)                                                                                        //catch errors
            {
                SNPService.DiagnosticOut(ex.ToString(), 1);                                                             //log them
            }
        }
        ...
        public void Deafen(string message)
        {
            try
            {
                ...
                SNPService.DiagnosticOut("Listening Has been set to" + receivedPacket["ListenBool"].ToString(), 1);     //logit
                if (Convert.ToInt32(receivedPacket["IntTimeInSeconds"] ?? 0) != 0)                                      //if inttime inseconds is set and not 0
                {
                    ...
                    if (OldSetting)
                        SNPService.DiagnosticOut("Listening Has been set to 1", 1);
                    else
                        SNPService.DiagnosticOut("Listening Has been set to 0", 1);
                }
            }
            catch (Exception ex)                                                                                        //catch the errors
            {
                SNPService.DiagnosticOut(ex.ToString(), 1);                                                             //let it log it
            }
        }
    }
}