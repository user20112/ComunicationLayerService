    ...
    public partial class PacLiteService : ServiceBase
    {
        ...
        public void DiagnosticOut(string message, int LoggingLevel)
        {
            try
            {
                ...
                {
                    using (StreamWriter DiagnosticWriter = File.AppendText(ConfigurationManager.AppSettings["DiagnosticFile"]))// @"C:\Users\d.paddock\Desktop\Diagnostic.csv"))
                    {
                        DiagnosticWriter.WriteLine(DiagnosticMessage);
                    }
                }
            }
            catch(Exception ex)
